...
try{
 ...
 const shot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.5.png',Buffer.from(shot,'base64'));
 ...
 const expandedArrow=await settings.locator('#custom-buttons>span:last-child').evaluate(e=>getComputedStyle(e).transform);assert.equal(expandedArrow,'matrix(0, 1, -1, 0, 0, 0)');await settings.locator('#custom-buttons').click();await settings.waitForTimeout(180);assert.equal(await settings.locator('#custom-buttons').getAttribute('aria-expanded'),'false');assert.equal(await settings.locator('#custom-buttons>span:last-child').evaluate(e=>getComputedStyle(e).transform),'none');
 ...
 const customShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.5.png',Buffer.from(customShot,'base64'));
 ...
 await feed(128,neutral);await new Promise(r=>setTimeout(r,620));await feed(128,neutral);await read();assert.equal(mouse[5],1,'R3 holds middle button');await feed(0,neutral);await read();assert.equal(mouse[5],0);assert.ok(mouse[3]>0&&mouse[4]>0,'fixture receives middle down and up');
 ...
 await overlay.waitForTimeout(250);const dualShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.overlay).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/双转盘-v05.5.png',Buffer.from(dualShot,'base64'));
 await feed(192,neutral);await feed(0,neutral);assert.equal(await app.evaluate(()=>global.__test.state().active),false);
 await read();const beforeTap=mouse[3];await feed(128,neutral);await feed(0,neutral);await new Promise(r=>setTimeout(r,650));await feed(0,neutral);await read();assert.equal(mouse[3],beforeTap+1,'solo short R3 still clicks middle');assert.equal(mouse[5],0);
 ...
 assert.equal(await settings.locator('#chord-interval').getAttribute('type'),'hidden');assert.equal(await settings.locator('#chord-interval').inputValue(),'300','gesture timing is fixed at 300 ms');
 ...
 await settings.locator('main').evaluate(e=>e.scrollTop=0);const controllerShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/手柄设置-v05.5.png',Buffer.from(controllerShot,'base64'));
 await settings.locator('[data-tab="appearance"]').click();await settings.locator('main').evaluate(e=>e.scrollTop=0);const finalShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.5.png',Buffer.from(finalShot,'base64'));
 console.log('PASS physical SendInput A/B operate settings, inert preview, fixed 300 ms gesture and auxiliary binding editor.');
