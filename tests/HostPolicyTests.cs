...
class PolicyTests {
 static int Main(){try{
  ...
  var s2=HostPolicy.NormalizeSystem(new Dictionary<string,object>{{"locale","invalid"},{"theme",false},{"startAtLogin","true"},{"closeBehavior","delete"}});Check((string)s2["locale"]=="zh-CN"&&!(bool)s2["startAtLogin"]&&(string)s2["closeBehavior"]=="tray","normalize invalid config");
