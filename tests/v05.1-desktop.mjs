...
try{
 ...
 const shot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.1.png',Buffer.from(shot,'base64'));
 ...
 const customShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.1.png',Buffer.from(customShot,'base64'));
 console.log('PASS editable bindings, Figma blue outline, custom colors and restore defaults.');

 target=spawn(path.resolve('native/bin/InputTarget.exe'),[],{windowsHide:false,stdio:['pipe','pipe','pipe']});let handle='',text='',shift=false,ctrl=false,mouseOnTarget=false,media=0,vertical=0,reads=0,mouse=[];
 createInterface({input:target.stdout}).on('line',l=>{if(l.startsWith('READY:'))handle=l.slice(6);if(l.startsWith('TEXT:')){text=Buffer.from(l.slice(5),'base64').toString();reads++;}if(l.startsWith('MOUSE:'))mouse=l.slice(6).split(',').map(Number);if(l.startsWith('SHIFT:'))shift=l.slice(6)==='True';if(l.startsWith('CTRL:'))ctrl=l.slice(5)==='True';if(l.startsWith('VERTICAL:'))vertical=Number(l.slice(9));if(l.startsWith('MEDIA:'))media=Number(l.slice(6));if(l.startsWith('MOUSEONTARGET:'))mouseOnTarget=l.slice(14)==='True';});
 ...
 await overlay.waitForTimeout(250);const dualShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.overlay).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/双转盘-v05.1.png',Buffer.from(dualShot,'base64'));
 ...
 await settings.locator('main').evaluate(e=>e.scrollTop=0);const controllerShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/手柄设置-v05.1.png',Buffer.from(controllerShot,'base64'));
 await settings.locator('[data-tab="appearance"]').click();await settings.locator('main').evaluate(e=>e.scrollTop=0);const finalShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.1.png',Buffer.from(finalShot,'base64'));
