...
try{
 ...
 const shot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.3.png',Buffer.from(shot,'base64'));
 ...
 const customShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.3.png',Buffer.from(customShot,'base64'));
 ...
 await overlay.waitForTimeout(250);const dualShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.overlay).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/双转盘-v05.3.png',Buffer.from(dualShot,'base64'));
 ...
 await settings.locator('main').evaluate(e=>e.scrollTop=0);const controllerShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/手柄设置-v05.3.png',Buffer.from(controllerShot,'base64'));
 await settings.locator('[data-tab="appearance"]').click();await settings.locator('main').evaluate(e=>e.scrollTop=0);const finalShot=await app.evaluate(async({BrowserWindow})=>(await BrowserWindow.fromId(global.__test.settings).webContents.capturePage()).toPNG().toString('base64'));await writeFile('../../outputs/设置面板-v05.3.png',Buffer.from(finalShot,'base64'));
