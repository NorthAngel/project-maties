...
$displayVersion = 'v' + (($version -split '\.')[0..1] -join '.')
...
$appDir = Join-Path $build "ConrollerPlus-$displayVersion"
...
try {
 ...
 foreach ($file in @('ConrollerPlus.exe','ConrollerPlus.exe.config','Microsoft.Web.WebView2.Core.dll','Microsoft.Web.WebView2.Wpf.dll','WebView2Loader.dll')) { Copy-Item -LiteralPath (Join-Path $hostBin $file) -Destination $appDir }
 ...
 foreach ($file in @('activation.js','device-profiles.js','repeat-actions.js','figma-assets.js','settings-copy.js','settings-controls.js','wheel-motion.js','appearance.js','controls.js','figma-layout.js','i18n.js','overlay.js','overlay.css','pointer.js','radial.js','runtime-engine.js','runtime-entry.js','webview-bridge.js','session.js','settings.js','settings.css','system.js','theme.js','wheel-view.js','wheel.css')) { Copy-Item -LiteralPath (Join-Path $repo "src/$file") -Destination (Join-Path $web 'src') }
 foreach ($file in @('app-icon.ico','tray-disconnected.ico','tray-light.ico','tray-dark.ico','app-icon.png','app-icon.svg','appearance-icon.svg','controller-icon.png')) { Copy-Item -LiteralPath (Join-Path $repo "assets/$file") -Destination (Join-Path $web 'assets') }
 foreach ($folder in @('exact','figma-ui','fonts','controller-icons')) { Copy-Item -LiteralPath (Join-Path $repo ('assets/'+$folder)) -Destination (Join-Path $web 'assets') -Recurse }
 ...
 Copy-Item -LiteralPath (Join-Path $repo 'docs/预览版使用说明.md') -Destination $appDir
 ...
 'Local preview: update installation and full repair are not yet available.' | Set-Content -LiteralPath (Join-Path $appDir 'preview-build.txt') -Encoding utf8
 $files = [ordered]@{}
 ...
 @{product='ConrollerPlus';channel='preview';version=$version;host='webview2';files=$files} | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $appDir 'release-manifest.json') -Encoding utf8
 $zip = Join-Path $destination "ConrollerPlus-$displayVersion-$stamp-win-x64.zip"
