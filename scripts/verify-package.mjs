...
import {FIGMA_ASSETS} from '../src/figma-assets.js';
...
if(manifest.product!=='ConrollerPlus'||manifest.host!=='webview2')throw Error('Wrong package identity');
const expected=new Set(['ConrollerPlus.exe','ConrollerPlus.exe.config','Microsoft.Web.WebView2.Core.dll','Microsoft.Web.WebView2.Wpf.dll','WebView2Loader.dll','recovery.zip','web/settings.html','web/overlay.html','web/native-translations.json','native/ControllerBridge.exe','native/SDL3.dll','native/gamecontrollerdb.txt']);
const dependencies=['activation.js','device-profiles.js','repeat-actions.js','figma-assets.js','settings-copy.js','settings-controls.js','wheel-motion.js','appearance.js','controls.js','figma-layout.js','i18n.js','overlay.js','overlay.css','pointer.js','radial.js','runtime-engine.js','runtime-entry.js','webview-bridge.js','session.js','settings.js','settings.css','system.js','theme.js','wheel-view.js','wheel.css'];
...
for(const item of Object.values(FIGMA_ASSETS))expected.add('web/'+item.src.replace(/^\.\//,''));
for(const f of ['SF-Pro.ttf','PingFang-SC.otf','Hiragino.otf'])expected.add('web/assets/fonts/'+f);
for(const f of ['glass-low.svg','glass-high.svg','language-normal.svg','language-active.svg','language-active-dark.svg'])expected.add('web/assets/figma-ui/'+f);
for(const f of ['tray-disconnected.ico','tray-light.ico','tray-dark.ico'])expected.add('web/assets/'+f);
let bytes=0;
...
await walk(root);
// Bundled Figma fonts are included in the release hashes; the old font-free size budget does not apply.
const executable=await readFile(path.join(root,'ConrollerPlus.exe'));const pe=executable.readUInt32LE(60);if(executable.readUInt16LE(0)!==0x5a4d||executable.readUInt16LE(pe+4)!==0x8664)throw Error('Host is not Windows x64');
